...
def do_crawling(image_list_path: str,prefix:str,index:str):
    """
    Main function to crawl metadatas with multi-threads.
    """
    global db
    db = database(prefix,index)
...
def worker(raw_image: str, index: int, num_images: int) -> list:
    ...
    if db.if_exist(namespace,image):
        logging.info(f"[{index + 1}/{num_images}] Image {namespace}/{image} existed. Pass.")
    ...
    while retry < max_retries:
        try:
            ...
            if content is None:
                logging.info(f"[{index + 1}/{num_images}] [404] {namespace}/{image} has been deleted")
                logging.info(f"[{index + 1}/{num_images}] {image_name_full} processed")
                return [index, raw_image]

            result['description'] = content['description']
            try:  # FIXED: Occasionally, when an image has a very brief description, it may not have a
            ...
            if retry == max_retries:
                logging.exception(
                    f"[{index + 1}/{num_images}] Image {image_name_full} get wrong resp while getting description; stop retrying {retry}")

                break
            logging.warning(
                f"[{index + 1}/{num_images}] Image {image_name_full} get wrong resp while getting description; retrying {retry}: URL is https://hub.docker.com/v2/repositories/{namespace}/{image}/")
    ...
    for tag in result['tags']:
        result['images'][tag] = get_images_by_tag(namespace, image, tag,index+1,num_images)

    # write results to database
    db.add(result,index+1,num_images)
...
def get_tags(namespace, image) -> list[str]:
    ...
    while flip_flag:
        ...
        try:
            ...
            logging.info(
                f"Image {namespace}/{image} get wrong resp while getting tags; retrying; URL is "
...
def get_images_by_tag(namespace, image, tag,index,image_nums) -> list[dict]:
    """
        ...
        try:
            ...
            for image_obj in content:
                ...
                if 'digest' not in image_obj:
                    logging.info(f"[{index}/{image_nums}] {namespace}/{image}:{tag} is empty")
                    tmp['digest'] = None
                else:
                    tmp['digest'] = image_obj['digest']
                if 'os' not in image_obj:
                    logging.info(f"[{index}/{image_nums}] {namespace}/{image}:{tag} has no os")
                    tmp['os'] = None
                else:
                    tmp['os'] = image_obj['os']
                if "layers" not in image_obj:
                    logging.info(f"[{index}/{image_nums}] {namespace}/{image}:{tag} has no avaliable layers")
                else:
            ...
            if retry == max_retries:
                logging.exception(
                    f"Image {namespace}/{image}:{tag} get wrong resp while getting building history; retrying {retry}")
                break
            logging.warning(
                f"Image {namespace}/{image}:{tag} get wrong resp while getting building history; retrying {retry}; URL is https://hub.docker.com/v2/repositories/{namespace}/{image}/tags/{tag}/images")
            retry += 1
    return result
